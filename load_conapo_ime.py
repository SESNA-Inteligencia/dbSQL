...
cursor.execute('DROP TABLE IF EXISTS conapo_IME')
# Generación de tabla
cursor.execute("""create TABLE conapo_IME(
    id INT,
    year INT,
    cve_ent INT,
    pob_tot INT,
    ...
    PRIMARY KEY (id),
    KEY (cve_ent),
    CONSTRAINT Fk_conapo1 FOREIGN KEY (cve_ent) REFERENCES inegi(cve_ent) on delete cascade
)
""")
db.commit()
...
base = pd.read_csv('./bases/IME.csv', encoding = 'utf8')
# procesamiento
...
#  mysql
query = """INSERT INTO conapo_IME(id, year, cve_ent, pob_tot, 
    analf, sbasc, ovsde, ovsee, ovsae, ovpt, vhac, pl5000, po2sm, im, gm, imn) 
...
for i, row in base.iterrows():
    ...
    values = (id[0], year[0], ent[0], pob_total[0],analf[0],sbasc[0],
